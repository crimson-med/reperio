import { anchorTagInclusive, attributeSeperator, bodyTagInclusive, footerTagInclusive, headTagInclusive, imageTagInclusive, linkTagInclusive, metaTagInclusive, scriptTagInclusive, styleTagInclusive, titleTagInclusive, videoTagInclusive } from "./classes/Expressions"
...
export class Parser {
    ...
    extractUrls(removeDuplicates = true) {
        const images = this.parsedPage.media.images.map(i => { if (i.src !== undefined) return i.src })
        const videos = this.parsedPage.media.videos.map(i => { if (i.src !== undefined) return i.src })
        const links = this.parsedPage.links.links.map(i => { if (i.href !== undefined) return i.href })
        const anchors = this.parsedPage.links.anchors.map(i => { if (i.href !== undefined) return i.href })
        const scripts = this.parsedPage.scripts.map(i => { if (i.src !== undefined) return i.src })
        const allResults = images.concat(videos, links, anchors, scripts).filter(val => val !== undefined)
        if (removeDuplicates) {
    ...
    extractImages(downloadLocation: string, removeDuplicates = true) {
        return new Promise<ExtractedFiles>((resolveDownload, rejectDownload) => {
            ...
            let images = this.parsedPage.media.images.map(i => { if (i.src !== undefined) return i.src })
            if (removeDuplicates) {
...
export const parserFromUrl = (url: string) => {
    return new Promise<{ error: undefined, parser: Parser } | { error: Error, parser: undefined }>((resolve, reject) => {
...
export const findSentenceWithWord = (payload: string, searchedTerm: string) => {
    ...
    const pattern = new RegExp(`${searchPatternStart}${searchedTerm}${searchPatternEnd}`, 'igm')
    let results: string[] = []
